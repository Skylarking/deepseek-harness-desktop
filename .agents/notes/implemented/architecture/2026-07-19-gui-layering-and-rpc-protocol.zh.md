...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）通过操作系统分配的回环端口承载同一套 Web 组合；其[所属决策](2026-08-16-electron-desktop-host-and-plugin-management.md)把 IPC fetch 载体保留为后续传输替换。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器与当前 Desktop 回环访问；零 workspace 依赖（注册表经结构注入到达） |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web，vite 应用）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发 + 每个应用一个拼装模块（web.ts / headless.ts）；vite 应用是 `dsh-client-web` 壳表面之上的薄 main；Desktop 持有 Electron 与其 DSH 子进程 | 各应用使用动态 import，因此不会互相加载；dist 与暂存 runtime 位置等 workspace 知识留在 app |
...
各应用保持这一区分：Web 应用挂载 Host、载体与浏览器组合；Desktop 在回环地址上监管该 Web 组合；`dsh --profile headless` 则挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。
...
| IPC 桥子类（假想示例——尚无此载体） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
