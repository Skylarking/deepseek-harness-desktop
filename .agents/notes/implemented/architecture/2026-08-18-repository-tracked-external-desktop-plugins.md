...
- **External plugin source remains in this repository.** `plugins/workspace-files`, `plugins/workspace-console`, and `plugins/codex-pets` are Git-tracked development sources. Each Workspace plugin root is the installable bundle and contains its Host and Client packages under `packages/`. Plugin-owned package names and Remote identifiers use the `@skylarking` scope; `@deepseek-ai` names in their manifests identify upstream Harness dependencies and the reversible official-layout alias. Their workspace membership supports repository builds without making them CLI or Desktop release dependencies.
