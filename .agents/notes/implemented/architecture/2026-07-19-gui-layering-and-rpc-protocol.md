...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) hosts the same Web composition over an OS-assigned loopback port; its [owning decision](2026-08-16-electron-desktop-host-and-plugin-management.md) keeps an IPC fetch carrier as a later transport replacement.

```
apps/*  (applications: web = vite app, cli = bin dispatch, desktop = Electron host)
  │ consume
  ▼
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser and current Desktop loopback access; zero workspace dependencies (the registry arrives by structural injection) |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web, the vite application) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch + one assembly module per application (web.ts / headless.ts); the vite app is a thin main over the `dsh-client-web` shell surface; Desktop owns Electron and its DSH child | Applications use dynamic imports so they never load each other; workspace knowledge like dist and staged-runtime locations stays in the app |
...
The applications preserve the division: the Web application mounts Host, carrier, and browser composition; Desktop supervises that Web composition on loopback; and `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (hypothetical example — no such carrier exists) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
