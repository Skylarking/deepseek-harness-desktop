...
|---|---|---|---|---|
| dsh | `packages/*/*` + `apps/cli` + `apps/web` | one version for the family and the workspace root, `0.0.x` | `dsh-v<version>` | `release.yml` |
...
| release-set boundary | every member of `packages/*/*` and `vendor/*`, plus `apps/cli` and `apps/web`; private applications such as `apps/desktop` remain outside npm publication |
