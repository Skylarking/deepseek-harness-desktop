...
Workspace entity registry (`ctx.workspaceRegistry`) for the DeepSeek Harness: durable workspace records, mutable primary directories, stable workspace order, and explicit session ownership stored through the domain data form. Consumers see the `Workspace` interface; the entity implementation stays package-private.
...
- `ctx.workspaceRegistry.setPath(id, path)` — replaces the current primary directory after canonicalizing and validating it. No two workspaces may own the same primary path. The stable workspace id, title, order, and session account remain unchanged.
...
- `Workspace.attachSession(id)` — prepends a known live or persisted session id independently of its immutable header cwd. A session already owned by another workspace and an unknown session reject without writing. Cross-workspace ownership changes are serialized. `detachSession` removes only the ownership entry.
...
- `Workspace.sessionIds` — the durable ordered ownership account. Changing the primary path does not rewrite old session headers or remove those sessions. A medium assigning one session to two workspaces, claiming one primary path from two records, or diverging from durable workspace order rejects at startup.
