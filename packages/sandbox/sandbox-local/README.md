...
The bwrap, Landlock, and Seatbelt profiles grant every canonical project root carried by a `workspace-write` policy. Seatbelt is allow-default with `(deny file-write*)` plus write allow-lists: `read-only` grants the `/dev/null` literal alone; `workspace-write` adds the primary and attached project roots, `/tmp`, and the per-user darwin temp dir (`os.tmpdir()` — the platform's real temp area for mkstemp-family tools). Apple marks the `sandbox-exec` CLI deprecated but ships it on every macOS; the functional probe is what fails closed if that ever changes.

The Windows rung keeps one deterministic write SID for the primary workspace and applies its standing ACE to the complete project-root set, but gives every live session/project-root-set pair a random private temp directory with a distinct SID and revocable ACE. Sessions sharing that set therefore share its intended write authority without inheriting one another's temp authority. A fresh provider always chooses a new temp path and SID, so crash residue cannot block or authorize a resumed session; agentless calls receive the same per-invocation isolation from the runner. Any project root equal to or containing the platform temp root fails before ACL mutation because its inheritable ACE would otherwise reach every private temp child.
