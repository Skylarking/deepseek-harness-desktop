...
bwrap、Landlock 和 Seatbelt profile 会授权 `workspace-write` 策略携带的每个规范项目根目录。Seatbelt profile 默认允许，但带 `(deny file-write*)` 和写入 allow-list：`read-only` 只授予 `/dev/null` 字面路径；`workspace-write` 另加主要与附加项目根目录、`/tmp` 和逐用户 darwin 临时目录（`os.tmpdir()`，即平台供 mkstemp 家族工具使用的真实临时区域）。Apple 将 `sandbox-exec` CLI（命令行界面）标为 deprecated，但所有 macOS 系统仍会提供它；若情况发生变化，功能探测会使执行被拒绝。

Windows 档为主要工作区保留一个确定性写入 SID，并将其常驻 ACE 应用于完整项目根集合；同时为每个活跃的会话／项目根集合分配一个随机私有临时目录，以及不同的 SID 和可撤销 ACE。因此，共享该集合的会话会共享预期的写权限，却不会继承彼此的临时目录权限。新的提供方总会选择新的临时路径和 SID，因此崩溃残留既无法阻止恢复的会话，也无法向其授权；runner 会为无 agent（智能体）的调用提供同样的逐调用隔离。如果任一项目根等于或包含平台临时根目录，调用会在 ACL 改动前失败，因为否则其可继承 ACE 会延伸到每个私有临时子目录。
