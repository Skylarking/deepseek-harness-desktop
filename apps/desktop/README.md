...
Plugins may declare a constrained Desktop overlay in their package manifest under `dsh.desktop.overlay`. The plugin owns its collapsed and expanded dimensions and its rendered Web surface. On macOS, the host creates a transparent non-focusable `panel`, keeps it above other applications and across Spaces and full-screen applications, and excludes it from Mission Control. A sandbox-compatible CommonJS preload exposes only movement and expansion. Overlay creation remains manifest-driven and the host destroys the window before replacing its runtime or when its plugin leaves the profile, so lifecycle changes leave no overlay state behind. Ordinary official Web windows do not receive this bridge.
